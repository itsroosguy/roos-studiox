/* ==========================================================================
   ROOS STUDIOX — FUTURE SYSTEMS NEURAL & QUANTUM ECOSYSTEM ENGINE
...
(function () {
  ...
  // Sampled SVG Path Contour Data from Roos StudioX Sovereign Mark
  ...
  class FutureSystemsEngine {
    constructor(canvasId) {
      ...
      // Color Palette (Restrained, Premium Luxury Tech)
      this.colors = [
        '#FFFFFF', // Soft White
        '#E6F0FF', // Ice Blue
        '#3B82F6', // Electric Blue
        '#6366F1', // Royal Violet
        '#22D3EE'  // Subtle Cyan Highlight
      ];
    ...
    samplePointsFromSVG() {
      ...
      // Sample 100 points along Path 1 and 35 along Path 2 (Total 135 contour nodes)
      createSampledPath(PATH_DATA_1, 100);
      ...
      // Map nodes in [0..1] unit space + 18 interior neural bridge nodes
    ...
    initDataParticles(count) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        this.dataParticles.push({
          ...
          color: Math.random() > 0.35 ? '#22D3EE' : '#3B82F6'
        });
      }
    }
    ...
    addEventListeners() {
      ...
      targetArea.addEventListener('click', (e) => {
        const rect = this.canvas.getBoundingClientRect();
        this.triggerQuantumRipple(e.clientX - rect.left, e.clientY - rect.top);
      });
    ...
    update(time) {
      this.ringAngle1 += 0.002;
      this.ringAngle2 -= 0.0015;
      ...
      // 12s Organic Breathing Wave
      const breathingScale = 1 + 0.025 * Math.sin(time * 0.0004);

      const centerX = this.width / 2;
      const centerY = this.height / 2;

      // Update Quantum Energy Ripples
      ...
      this.nodes.forEach(node => {
        ...
        // Check impact from click ripples
    ...
    render() {
      ...
      // 1. Draw Volumetric Light Aura
      const ambientGrad = this.ctx.createRadialGradient(centerX, centerY, 10, centerX, centerY, Math.max(this.width, this.height) * 0.45);
      ambientGrad.addColorStop(0, 'rgba(59, 130, 246, 0.15)');
      ambientGrad.addColorStop(0.5, 'rgba(99, 102, 241, 0.06)');
      ambientGrad.addColorStop(1, 'rgba(10, 10, 10, 0)');
      this.ctx.fillStyle = ambientGrad;
      this.ctx.fillRect(0, 0, this.width, this.height);

      // 2. Draw Holographic Quantum Geometry Orbit Rings
      const ringRadius1 = Math.min(this.width, this.height) * 0.38;
      const ringRadius2 = Math.min(this.width, this.height) * 0.28;

      this.ctx.save();
      this.ctx.translate(centerX, centerY);

      // Outer Ring
      this.ctx.rotate(this.ringAngle1);
      this.ctx.beginPath();
      this.ctx.arc(0, 0, ringRadius1, 0, Math.PI * 2);
      this.ctx.strokeStyle = 'rgba(59, 130, 246, 0.08)';
      this.ctx.setLineDash([8, 24]);
      this.ctx.lineWidth = 1;
      this.ctx.stroke();

      // Inner Ring
      this.ctx.rotate(this.ringAngle2);
      this.ctx.beginPath();
      this.ctx.arc(0, 0, ringRadius2, 0, Math.PI * 2);
      ...
      // 3. Draw Quantum Energy Ripples
      this.ripples.forEach(r => {
        this.ctx.beginPath();
        this.ctx.arc(r.x, r.y, r.radius, 0, Math.PI * 2);
        this.ctx.strokeStyle = `rgba(34, 211, 238, ${r.alpha.toFixed(3)})`;
        this.ctx.lineWidth = 1.5;
        this.ctx.stroke();
      });

      // 4. Draw Synapse Connections
      this.edges.forEach(edge => {
        ...
        const lineAlpha = (1 - dist / (edge.maxDist * 1.3)) * 0.26 * Math.min(n1.alpha, n2.alpha);
      ...
      // 6. Draw Intelligent Glowing Nodes
      this.nodes.forEach(node => {
        if (node.alpha > 0.6) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => new FutureSystemsEngine('heroNeuralCanvas'));
  } else {
    new FutureSystemsEngine('heroNeuralCanvas');
  }
